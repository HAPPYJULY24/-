...
* **Upper Bound (>) & Lower Bound (<) (入场上下轨)**：信号触发器阈值（例如 `0.50` 与 `-0.50`）。当因子值跨越该界限时产生交易指令。
* **Risk Target (%) (波动率风控目标)**：
...
* **Exec Mode (执行模式 - 极关键)**：
  * **`Close (T+1)` (收盘执行)**：基于 $t$ 时刻的因子值，在 $t+1$ 时刻的收盘价执行。主要用于粗颗粒日线研究。
  * **`Next Open (T+1)` (次日开盘执行 - 强力推荐)**：基于 $t-1$ 时刻的因子值，在 $t+1$ 时刻的开盘价执行。**这是业界最严苛的“Look-ahead Free (防前瞻)”真实交易环境模拟**，彻底消除回测中因“信号产生➔市价下单”微秒级时间偏差引起的未来数据泄露。
