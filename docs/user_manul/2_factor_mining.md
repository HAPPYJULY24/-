...
* **Select Risk Factors (选择风险因子)**：
  ...
  * **业务原理**：因子常常与资产的价格水平、波动率或成交量高度相关（例如“动量因子”天然带有很高的“波动率偏向”）。中性化利用 **Ridge (岭回归)** 正则化算法，将您的因子作为因变量，选中的风险因子作为自变量进行回归，**提取残差值作为纯净的“风险中性化因子”**。
* **Ridge Alpha (岭回归正则化系数)**：默认 `1.00`。用于控制线性残差提取时的惩罚强度，稳定共线性风险因子矩阵的残差解。
...
* **业务解读**：
  ...
  * **风控预警**：若中性化因子与某一风险因子（如 `close`）的相关系数绝对值依然高达 `0.7` 以上，说明 Ridge 中性化正则惩罚过多或因子本身完全退化为价格的线性组合，需要警惕风险暴露。
